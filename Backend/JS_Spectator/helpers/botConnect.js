...
var ChatBot = function(username, password, options) {
  ...
  // Live Game Data information.
  this.currentRequest;  // If it has a currentRequest, bot is 'busy'.
  this.requestQueue = [];
...
ChatBot.prototype.addLiveGameDataRequest = function (request) {
  this.requestQueue.push(request);

  // TODO: Move this to app.js and create a request handler.
...
ChatBot.prototype.processRequest = function () {
  if (this.requestQueue.length > 0 && !this.currentRequest)
  {
    this.currentRequest = this.requestQueue.splice(0, 1)[0];
    this.spectateFriendGame(this.currentRequest);
  }
};
